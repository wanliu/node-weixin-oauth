...
var oauth = {
   ...
   * @returns {string}
   */
  buildUrl: function (params, options) {
    var oauthUrl
  ...
  createURL: function (appId, redirectUri, state, scope, type) {
    ...
    var params = {
      ...
      scope: ['snsapi_base', 'snsapi_userinfo', 'snsapi_login', 'web_wx_login'][scope],
      state: state
    };
    return this.buildUrl(params);
  },

  createQRCodeURL: function (appId, redirectUri, state, scope, type) {
    assert((scope >= 0) && (scope <= 3));
    assert(state !== null);
    type = 0;
    var params = {
    ...
    return this.buildUrl(params, {qrcode: true});
  },
  /**
